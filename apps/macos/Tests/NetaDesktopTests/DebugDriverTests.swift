...
final class DebugDriverTests: XCTestCase {
	func testEveryCommandFormParses() throws {
		let cases: [(String, DebugCommand)] = [
			...
			("await-state 5000 terminal-ready", .awaitState(milliseconds: 5000, predicate: "terminal-ready")),
			("await-state 5000 terminal-text Mission created", .awaitState(milliseconds: 5000, predicate: "terminal-text Mission created")),
			...
			("menu", .menu),
			("state", .state),
			("terminal-paste Create a bounded mission", .terminalPaste(text: "Create a bounded mission")),
			("ax-dump", .axDump),
	...
	func testGarbageIsRefusedWithAReason() {
		let bad = [
			...
			"await-state 5000 terminal-text",
			"terminal-paste",
			"shot",
			"shot-content",
			"fit now",
