...
final class NavigatorTests: XCTestCase {
	...
	func testSingleMachineGivesNilMachines() async throws {
		...
		let row = try XCTUnwrap(workspaceRows(overlay(model).sections(query: "")).first)
		XCTAssertEqual(row.machines.map(\.status), ["Online"])
		store.setNodeOffline(true)
		let offline = NavigatorModel.make(store: store, query: "")
		let offlineRow = try XCTUnwrap(workspaceRows(overlay(offline).sections(query: "")).first)
		XCTAssertTrue(offlineRow.machines.isEmpty)
	}

	func testWorkspaceSearchTrimsAndIgnoresCase() throws {
		let rows = try workspaceRows(
			overlay(NavigatorModel.make(store: twoWorkspaceStore(), query: ""))
				.sections(query: "  OTH  "))
		XCTAssertEqual(rows.map(\.name), ["other"])
	}

	func testConnectedMachineIsNestedUnderEveryWorkspaceWithALocalRoot() throws {
		let store = twoWorkspaceStore()
		store.setCurrentWorkspace(otherWorkspaceId)
		let rows = try workspaceRows(overlay(NavigatorModel.make(store: store, query: "")).sections(query: ""))
		XCTAssertEqual(rows.count, 2)
		XCTAssertTrue(rows.allSatisfy { $0.machines.map(\.id) == ["m1"] })
	}

	func testUnknownRemoteRootIsNotPresentedAsOnline() throws {
		let store = storeFor(
			missions: [], roots: [WorkspaceRoot(machineId: "remote", path: "/vol/remote")])
		let row = try XCTUnwrap(
			workspaceRows(overlay(NavigatorModel.make(store: store, query: "")).sections(query: "")).first)
		XCTAssertTrue(row.machines.isEmpty)
	}
	...
	func testNoLeaderRowAndNoGroupCounts() async throws {
		...
		XCTAssertEqual(
			labels,
			["archived", "connectedMachine", "machines", "onlineMachineId", "open", "selectedWorkspaceId", "workspaces"])
