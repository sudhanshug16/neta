...
When an OpenCode actor ends a turn, the runtime sends an automatic report to its parent: workers to their mission lead, mission leads to the workspace leader. New missions require a separate lead task and effort (unless a model is explicit); `lead: "self"` is rejected before creation. Historical self-led missions remain readable and closeable, but cannot resume active work. The report includes the selected model and final transcript excerpt, including any failure cause. It uses the durable inbox to wake an idle parent or queue behind active work. Unacknowledged deliveries persist on the actor and retry; duplicate turn reports are suppressed. A normal turn ending without explicit completion marks the actor idle and resumable; cancellation marks it interrupted; a provider error marks it failed. Neither marks the mission complete. A mission lead calls `neta_ready` without an ID to record its own completed handoff; the workspace leader uses the public mission number to close it. Successful checks close as `completed`, merges require commit evidence, and abandonment is reserved for intentionally discarded work. Repeating the same close is idempotent. Closed missions and archived actors do not wake parents. Automatic model switching is disabled, including previously persisted fallback lists.
