...
describe("models", () => {
	test.skipIf(!nativeHarnessReady)(
		"models --json lists OpenCode with one default: true",
		async () => {
			const h = await ensureSetup();
			...
			expect(defaults[0]).toMatchObject({ provider: "opencode", model: "test/test-model" });
	...
	test.skipIf(!nativeHarnessReady)(
		"models text marks the default model",
		async () => {
			const h = await ensureSetup();
...
describe("model", () => {
	test.skipIf(!nativeHarnessReady)(
		"model sets the leader session by OpenCode id",
		async () => {
			const h = await ensureSetup();
			const result = await h.run(["model", "test/test-model"]);
			expect(result.code).toBe(0);
			expect(result.stdout.trim()).toBe("model opencode/test/test-model");
		},
		120000,
	);

	test.skipIf(!nativeHarnessReady)(
		"model accepts a provider/model id",
		async () => {
			const h = await ensureSetup();
			const result = await h.run(["model", "opencode/test/test-model"]);
			expect(result.code).toBe(0);
			expect(result.stdout.trim()).toBe("model opencode/test/test-model");
		},
		120000,
	...
	test.skipIf(!nativeHarnessReady)(
		...
		async () => {
			...
			const edited = {
				...
				forbiddenModels: ["test/legacy-other"],
			};
			await writeFile(settingsPath, JSON.stringify(edited, null, "\t"));
			try {
				const result = await h.run(["model", "test/legacy-other"]);
				...
				expect(parsed.find((entry) => entry.model === "test/legacy-other")?.forbidden).toBe(true);
			} finally {
...
describe("resolveModel", () => {
	...
	test("an advertised model id containing a slash resolves", () => {
		expect(
			resolveModel([{ id: "test/test-model", name: "Fixture", provider: "opencode" }], "test/test-model"),
		).toEqual({
			provider: "opencode",
			id: "test/test-model",
		});
