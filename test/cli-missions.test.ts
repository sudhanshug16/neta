...
describe("missions list", () => {
	test("missions --json parses to open missions, newest first", async () => {
	...
	test("text truncates a long name to 32 columns and shows attention", async () => {
		const h = await ensureSetup();
	...
	test("--all includes the closed mission plain missions omits", async () => {
		const h = await ensureSetup();
...
describe("mission detail", () => {
	test("mission 1 prints the objective and its agents", async () => {
		const h = await ensureSetup();
	...
	test("mission 1 --json prints mission, agents and events", async () => {
		const h = await ensureSetup();
