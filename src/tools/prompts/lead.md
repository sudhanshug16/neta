...
`fallbackModels` remains an explicit ordered list of permitted connected IDs for failures before execution. Omit or pass [] to prohibit substitution. A failed connection or uncertain routing decision does not authorize upgrading effort or choosing a more expensive model.
